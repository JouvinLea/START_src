...
START::HandleResolArea::HandleResolArea(TString configname, TString mcprod, bool forceuseofdistribution, bool readnow)
  :fMcProductionName(mcprod),
  fDataSum(0),
  fBandArray(0),
  fForceUseDistribution(false),
  fIsDataSummaryFromDynamicAlloc(false),
  ...
  fEnergyBinThresholdCondition(Safe)
{
  ...
  MonteCarlo MC;
  fEffMin = MC.GetEfficiency().front();
  fEffMax = MC.GetEfficiency().back();
  fOffMin = MC.GetOffset().front();
  fOffMax = MC.GetOffset().back();
  fZenMin = MC.GetZenith().front();
  fZenMax = MC.GetZenith().back();
...
{
  if (fIsDataSummaryFromDynamicAlloc && (fDataSum!=0)) {
    delete fDataSum;
  }
 ...
 */
void START::HandleResolArea::SetAndUpdateBands(std::vector<Band> &BandArray,DataSummary *DataSum)
{
  

  if (fDataSum && fIsDataSummaryFromDynamicAlloc) {
    delete fDataSum;
    fDataSum=0;
  }
  ...
  if (DataSum!=0) 
  ...
  else 
    {
      ...
      fDataSum = new DataSummary(*fBandArray);
      fIsDataSummaryFromDynamicAlloc=true;
  ...
  // MC values
  fEffMin = fDataSum->GetPairEfficiency().first;
  fEffMax = fDataSum->GetPairEfficiency().second;
  fOffMin = fDataSum->GetPairOffset().first;
  fOffMax = fDataSum->GetPairOffset().second;
  fZenMin = fDataSum->GetPairZenith().first;
  fZenMax = fDataSum->GetPairZenith().second;
...
int START::HandleResolArea::ReadAndStore()
{
  ...
  // Get MC values
  MonteCarlo MC;  
  std::vector<double> effMC = MC.GetEfficiency();
  std::vector<double> offMC = MC.GetOffset();
  std::vector<double> zenMC = MC.GetZenith();
  std::vector<TString> telMC = MC.GetTelType();
  std::vector<double> enMC = MC.GetEnergy();

  // If an object datasummary exists, we take only the instruments
  // functions needed 

  std::vector<TString> telType;
  std::vector<TString>::iterator itel_begin;
  std::vector<TString>::iterator itel_end;

  if (fDataSum!=0) {
    telType = fDataSum->GetWhichTelType();
    itel_begin = telType.begin();
    itel_end = telType.end();
  }
  else {
    itel_begin = telMC.begin();
    itel_end = telMC.end();
  }
					 ...
					 std::vector<double> &xinter, std::vector<double> &yinter)
{
  ...
  MonteCarlo MC;
  double emin(MC.GetEnergy().front()), emax(MC.GetEnergy().back());
...
void START::HandleResolArea::DrawAllIRF()
{
  ...
  std::vector<double> veff = MC->GetEfficiency();
  std::vector<double> vzen = MC->GetZenith();
  std::vector<double> voff = MC->GetOffset();
  std::vector<TString> vteltype = MC->GetTelType();
...
void START::HandleResolArea::BuildIRFBiaisCanvas(double eff, double zen, double off, unsigned int telcode) {
  ...
  canvasbiaistitle+=MC.GetStringFromTelcode(telcode);
  ...
  if(fmapVectorsBiais[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].first.size()>0) {
    
    xbiais = fmapVectorsBiais[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].first;
    ybiais = fmapVectorsBiais[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].second;
    ...
    fMapIRFBiaisGraph[graphname]->SetName(MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode)).Data());
    ...
    WARN_OUT << "Can't find map for biais :" << MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode)) << std::endl;
  }
...
void START::HandleResolArea::BuildIRFResolutionCanvas(double eff, double zen, double off, unsigned int telcode) {
  ...
  canvasresolutiontitle+=MC.GetStringFromTelcode(telcode);
  ...
  if(fmapVectorsResol[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].first.size()>0) {
    
    xresolution = fmapVectorsResol[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].first;
    yresolution = fmapVectorsResol[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].second;
    ...
    fMapIRFResolutionGraph[graphname]->SetName(MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode)).Data());
    ...
    WARN_OUT << "Can't find map for resolution :" << MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode)) << std::endl;
  }
...
void START::HandleResolArea::BuildIRFEffectiveAreaCanvas(double eff, double zen, double off, unsigned int telcode) {
  ...
  canvasareatitle+=off;
  canvasareatitle+=MC.GetStringFromTelcode(telcode);
  ...
  if(fmapVectorsArea[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].first.size()>0) {
    
    xarea = fmapVectorsArea[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].first;
    yarea = fmapVectorsArea[MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode))].second;
    ...
    fMapIRFEffectiveAreaGraph[graphname]->SetName(MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode)).Data());
    ...
    WARN_OUT << "Can't find map effective area :" << MakeKeyRun(eff,zen,off,MC.GetStringFromTelcode(telcode)) << std::endl;
  }
...
void START::HandleResolArea::BuildBandEffectiveAreaCanvas(Band &BandToDraw) {
  ...
  std::pair<double,double> paireff = MC.GetMCLimitingEfficiency(BandToDraw.GetEff());
  double effmin(paireff.first), effmax(paireff.second);

  std::pair<double,double> pairzen = MC.GetMCLimitingZenith(BandToDraw.GetZenON());
  double zenmin(pairzen.first), zenmax(pairzen.second);

  std::pair<double,double> pairoff = MC.GetMCLimitingOffset(BandToDraw.GetOffset());
 ...
 * 
 * For the fDataSum, BE CAREFULL ! The value is really copied (by a new instance), so if you give your DataSummary, then,
 * after the copy constructor, it will be a new instance, not linked any more to the one given by the user). TODO : Change this ? 
 *
 */
START::HandleResolArea::HandleResolArea(HandleResolArea const &StoreCopy)
  :TObject(StoreCopy),
   ...
   fIsDataSummaryFromDynamicAlloc(StoreCopy.fIsDataSummaryFromDynamicAlloc),
   ...
   fResolFactor(StoreCopy.fResolFactor)
{
  ...
  fDataSum = new DataSummary(*(StoreCopy.fDataSum));
  fIsDataSummaryFromDynamicAlloc = true; // VIM : Since we just have created a copy of the original vector...
...
START::HandleResolArea &START::HandleResolArea::operator=(HandleResolArea const& StoreCopy)
{
  if(this != &StoreCopy) {
    ...
    // VIM MERGE NOTE : Ces trois lignes, je ne sais pas trop....
    // Dans la maniere dont je l'utilise, il peut etre alloue dynamiquement
    // Ou par l'utilisateur....
    // Dois-je systematiquement en faire une nouvelle instance ? 
    // ou copier le pointeur ? 
    if(fDataSum!=0) {
      delete fDataSum;
    }
    fDataSum = new DataSummary(*(StoreCopy.fDataSum));
