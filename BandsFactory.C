 ...
 *
 */
START::BandsFactory::BandsFactory(const Config &config, bool verbose)
:fConfig(0),
								 fverbose(verbose)
{
...
<<" Very Big DEAD TIME " << LiveTimeFraction <<" (check if DB is correct)"<<std::endl;
 ...
 if(!fConfig->GetUserIsMc()) {
   TTree *EventsTTree_AllSelected = (TTree *)treefile->Get(STARTUtils::GetAllEventsTreeName());
 ...
 for(int zbin=0;zbin<RunNumberOfZBands;zbin++) {
   ...
   if(UseTwoZenBands==false) {
     if(nbOffBand1>0) BandArray[fNbBands].SetZenOFF((double)(BandArray[fNbBands].GetZenOFF()/nbOffBand1));
       ...
       WARNING <<"Run "<<fRunList[irun]<<" NoffBand="
	       << nbOffBand1 <<" => SKIP "<<BandOrRun <<std::endl;
       BandArray[fNbBands].SetKeepBand(0);
       continue;
     }
   }
     ...
     if(zbin==0 && nbOffBand1>0) BandArray[fNbBands-1].SetZenOFF((BandArray[fNbBands-1].GetZenOFF()/nbOffBand1));
       ...
       WARNING << "Run "<<fRunList[irun]<<" NoffBand1 = "
	       << nbOffBand1 <<" => SKIP "<<BandOrRun <<std::endl;
       BandArray[fNbBands-1].SetKeepBand(0);
       continue;
     }
     if(zbin==1 && nbOffBand2>0) BandArray[fNbBands].SetZenOFF((double)(BandArray[fNbBands].GetZenOFF()/nbOffBand2));
       ...
       std::cout <<"Run "<<fRunList[irun]<<"NoffBand2="
		 << nbOffBand2 <<" => SKIP "<<BandOrRun <<std::endl;
       BandArray[fNbBands].SetKeepBand(0);
       continue;
     }
   }
...
void START::BandsFactory::ReprojBands(std::vector<Band> &BandArray,std::map<TString, std::vector<int> > &InfoReprojArray, std::vector<Band> &ReprojArray,TString configname,HandleResolArea &HandleIRF)
{
  int nband=0;
